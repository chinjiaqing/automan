...
- **声明即变量**：输出变量在 IO 编辑器声明后自动注入 `ctx.variables`，内部 variable 节点可写入，引擎结束时提取
- **变量隔离**：片段执行前快照变量，结束后恢复父级变量并写入输出
- **作用域限制**：片段内变量强制 `local`，无全局/外部输入概念
- **节点限制**：片段内不允许 end/endSuccess/endFail/return/call 节点
- **输入参数**：通过 call 节点的 `arg_*` 配置注入，支持引用父级变量
- **输出参数**：自然结束时从变量池提取声明的输出变量值

| API | 说明 |
|-----|------|
| GET `/api/fragments` | 片段列表 |
| GET `/api/fragments/:id` | 获取单个片段 |
| POST `/api/fragments` | 新建片段 |
| PUT `/api/fragments/:id` | 更新片段 |
| DELETE `/api/fragments/:id` | 删除片段 |
| GET `/api/fragment-groups` | 分组列表 |
| POST `/api/fragment-groups` | 新建分组 |
| PUT `/api/fragment-groups/:id` | 重命名分组 |
| DELETE `/api/fragment-groups/:id` | 删除分组 |
...
│       │   └── migrate.ts    # 幂等 schema 同步 (CREATE TABLE IF NOT EXISTS)
│       ├── modules/
│       │   ├── actor/        # Actor 基类 + 管理器
│       │   ├── device/       # ADB 服务封装
│       │   ├── task/         # 任务 Actor 系统
...
│       │   ├── FragmentView.vue # 片段编辑器 (分组+片段列表+画布+配置面板) [BETA]
