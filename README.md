...
凹凸曼是一款面向 **安卓设备** 自动化任务的可视化工具。

通过 Web 页面管理多台安卓设备（真机 / 模拟器均可），使用拖拽式节点编辑器构建工作流——识图、OCR、点击、条件判断、循环、随机等待等，系统以截图为输入自动驱动工作流执行。
...
支持所有通过 ADB 连接的安卓设备，无需区分真机或模拟器。

| 设备类型 | 连接方式 | ADB 地址示例 |
|----------|----------|----------------|
| USB 真机 | USB 数据线 + USB 调试 | 序列号（如 `ABCDEF123456`） |
| WiFi 真机 | 无线调试 / adb tcpip | `ip:port`（如 `192.168.1.5:5555`） |
| 模拟器 | 自动识别 | `127.0.0.1:端口`（如雷电、MuMu 等） |

> **分辨率自适应**：系统自动获取设备分辨率并计算缩放比，支持任意分辨率的横屏 / 竖屏设备。所有坐标操作自动转换为设备实际坐标，无需手动设置。
>
> 添加设备时点击「扫描设备」可自动发现已连接的 USB / 模拟器设备，点击「测试连接」可验证连通性。
...
添加、编辑、删除设备，填写 ADB 地址即可绑定（支持扫描自动发现）。每台设备可独立运行多个工作流，互不干扰。设备运行状态实时监控。
...
每 2 秒自动截屏，截图上实时叠加执行注解（识别框、点击波纹、文字区域）。点击看板图片可直接操控设备，支持手动刷新截图。执行日志实时滚动显示。
...
首页左侧点击「添加设备」→ 输入别名 → 填写 ADB 地址（或点击「扫描设备」自动发现）→ 可选「测试连接」验证 → 确认创建。
